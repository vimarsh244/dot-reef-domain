...
address payable public owner;
  ...
  function register(string calldata name) public payable {
    ...
    require(msg.value >= _price, "Not enough $reef paid");
    ...
    string memory json = Base64.encode(
      abi.encodePacked(
        '{"name": "',
        _name,
        '", "description": "A domain on the Reef name service (for fun @vimarshtweets)", "image": "data:image/svg+xml;base64,',
  ...
  function price(string calldata name) public pure returns(uint) {
    ...
    require(len > 0);
    if (len == 3) {
      return 50 * 10**18; // 50 Reef = 50 000 000 000 000 000 000 (18 decimals). We're going with 0.5 Matic cause the faucets don't give a lot
    } else if (len == 4) {
      return 30 * 10**18; // To charge smaller amounts, reduce the decimals. This is 30
    } else {
      return 10 * 10**18;
    }
  }
